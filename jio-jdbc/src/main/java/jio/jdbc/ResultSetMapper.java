...
public interface ResultSetMapper<Entity> {
  ...
  /**
   * Mapper to handle a resul-set that contains at most one row. The `map` function
   * doesn't have to call the `next` method, just to collect the data from the column names.
  ...
  /**
   * Creates a ResultSetMapper to handle a result set where each row represents an entity. The `map` function
   * is called for each row to collect entities into a list. The `next` method doesn't need to be called
   * within the map function, as it is automatically handled by this mapper.
   *
