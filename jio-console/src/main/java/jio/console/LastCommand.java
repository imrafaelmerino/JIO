        ...
        super(COMMAND_NAME,
              """
                      ...
                      jio.chatgpt.Examples:
                          ...
                          $command 3
                          $command every 100
                          $command every 100 for 1000""".replace("$command", COMMAND_NAME)
             );
    }
    ...
    public Function<String[], IO<String>> apply(final JsObj conf,
        ...
        return tokens -> {
            ...
            if (pattern1.matcher(command).matches()) {
                ...
                for (int i = 1; i < parseInt(tokens[1]); i++) list = list.append(lastCommand);
                return list.map(it -> String.join("\n", it));
            }
            if (pattern2.matcher(command).matches())
            ...
            if (pattern3.matcher(command).matches())
                return lastCommand.then(s -> Programs.PRINT_NEW_LINE(s)
                                                     .map(it -> s)
                                       )
                                  .repeat(s -> true,
