...
public interface MyOauthHttpClient extends MyHttpClient {
    ...
    /**
     * HttpLambda that takes a request builder and returns a JIO effect with the http response, parsing the body with
     * the given handler and requesting an access token when necessary. There are different predefined http lambdas in
     * this class to parse the most common types: {@link #ofString strings} and {@link #ofBytes()} etc
     *
