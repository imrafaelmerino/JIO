...
public final class Report {
    ...
    void tac(Instant tic) {
        ...
        long duration = Duration.between(tic, Instant.now()).toNanos();
        if (duration > maxTime) {
            maxTime = duration;
        }
        if (duration < minTime) {
    ...
    public JsObj toJson() {
        return JsObj.of(
                "name", JsStr.of(propName),
                "n_tests", JsInt.of(tests),
                "n_failures", JsInt.of(failures.size()),
                "n_exceptions", JsInt.of(exceptions.size()),
                "description", JsStr.of(propDescription),
                "start_time", JsInstant.of(startTime),
                "end_time", JsInstant.of(endTime),
    ...
    Report aggregatePar(Report other) {
        ...
        result.accumulativeTime = Math.max(accumulativeTime, other.accumulativeTime);
    ...
    private Report aggregateCommon(Report other) {
        ...
        result.minTime = Math.min(minTime, other.minTime);
        result.maxTime = Math.max(maxTime, other.maxTime);
