...
/**
 * Represents a functional effect that encapsulates asynchronous computations, including successful results and failures.
 * This class models a computation that returns a {@link CompletableFuture} of type `O`, where `O` is the type of the
 * result the future is completed with when the computation succeeds. Functional effects are used to model asynchronous
 * operations in a composable and error-handling manner.
 *
 * <p>
 ...
 * allow you to create effects from different types of computations, such as lazy computations, tasks, resources, and more.
 *
 * <p>
 ...
 * <p>
 * To perform a blocking wait for the computation's result, you can use the {@link IO#result()} method, which blocks
 * the caller thread until the computation is complete and returns the result or throws a failure as a
...
public sealed abstract class IO<O> implements Supplier<CompletableFuture<O>> permits Delay, Exp, Val {
    ...
    /**
     * Creates an effect that always produces a result of null. This method is generic and captures
     * the type of the caller, allowing you to create null effects with different result types.
     *
     * <p>Example usage:
     * <pre>{@code
     * IO<String> a = NULL();
    ...
    /**
     * Creates an effect from a lazy computation that returns a CompletableFuture. This method
     * allows you to encapsulate an asynchronous operation represented by a lazy future into an IO effect.
     *
     ...
     * This method is designed to handle resources that implement the {@link AutoCloseable} interface,
     * ensuring proper resource management to avoid memory leaks.
     *
    ...
    /**
     * Creates an effect from a lazy computation. Every time the `get()` or `result` method is called,
     * the provided supplier is invoked, and a new computation is returned. Since a supplier cannot
     * throw exceptions, an alternative constructor {@link #task(Callable)} is available that takes
     * a {@link Callable callable} instead of a {@link Supplier supplier} if exception handling is needed.
     *
     ...
     * Every time the `get()` or `result` method is called, the provided task is executed asynchronously using the given executor.
     *
     * @param callable the callable task to be executed.
     ...
     * @param <O> the type parameter representing the result type of the effect (in this case, typically representing an exception).
    ...
    /**
     * Creates an effect from a lazy computation modeled with a {@link Supplier} and executes it asynchronously
     * using the specified {@link Executor}. Every time the `get()` or `result` method is called, the provided
     * lazy computation is executed asynchronously using the given executor.
     *
    ...
    /**
     * Creates an effect from a supplier using the Java interface {@link ForkJoinPool.ManagedBlocker ManagedBlocker}
     * to efficiently utilize the common Java fork/join thread pool.
     * <p>
     * Please note that every time the `get` or `result` methods are called on the returned effect, the `get` method
     * of the provided supplier is invoked, and a new computation is executed.
     * <p>
     * Also, be aware that the maximum default level of parallelism that can be achieved with the ForkJoin pool
     * is typically governed by the {@link ForkJoinPool#DEFAULT_COMMON_MAX_SPARES} constant, which can be
     * overridden using the "java.util.concurrent.ForkJoinPool.common.maximumSpares" system property.
     * Exceeding this limit may result in a {@link RejectedExecutionException} exception with the message
     ...
     * <p>
     * Please note that every time the `get` or `result` methods are called on the returned effect, the provided task
     * is executed, and a new computation is performed.
     * <p>
     * Also, be aware that the maximum default level of parallelism that can be achieved with the ForkJoin pool
     * is typically governed by the {@link ForkJoinPool#DEFAULT_COMMON_MAX_SPARES} constant, which can be
     * overridden using the "java.util.concurrent.ForkJoinPool.common.maximumSpares" system property.
    ...
    /**
     * Returns the first computation that completes, regardless of whether it fails or succeeds.
     * This method is implemented using {@link CompletableFuture#anyOf(CompletableFuture[])}, and its behavior
     * is inherited from that method. It's important to note that this operator is a good candidate to be
     * reimplemented with Fibers in the future to benefit from cancellation and non-returning failing computations.
     *
    ...
    /**
     * Creates a new effect that, when this succeeds, maps the computed value into another value using the specified function.
     *
    ...
    /**
     * Creates a new effect by applying the specified {@link Lambda} to the result of this effect (if it succeeds).
     * If this effect fails, the new effect also ends with the same failure, and the lambda is not applied.
     * This method is commonly referred to as "flatMap," "thenCompose," or "bind" in different programming languages
     * and libraries. For brevity, it's named "then" here.
     *
    ...
    /**
     * Creates a new effect by applying the specified {@link Lambda} to the result of this effect (if it succeeds).
     * The lambda is evaluated by a thread from the pool of the specified executor. If this effect fails,
     * the new effect also ends with the same failure, and the lambda is not applied. This method is commonly
     * referred to as "flatMap," "thenCompose," or "bind" in different programming languages and libraries.
     * For brevity, it's named "then" here.
     *
     * @param fn       the lambda that takes the result of this effect to create another one.
     * @param <Q>      the result type of the new effect.
     ...
     * Creates a new effect after evaluating this one. If this succeeds, the result is applied to the specified
     * successLambda. If this fails, instead of ending with a failure, the exception is applied to the specified
     * failureLambda to create a new result. In both cases, the lambdas are evaluated by a thread from the pool
     * of the specified executor.
     *
     * @param successLambda the lambda that takes the result to create another one in case of success.
     * @param failureLambda the lambda that takes the exception to create another result in case of failure.
    ...
    /**
     * Creates a new effect that will handle any failure that this effect might contain and will be recovered
     * with the value evaluated by the specified function. If this effect succeeds, the new effect will also succeed
     * with the same value. If this effect fails, the specified function is applied to the exception to produce
     * a new value for the new effect.
     *
    ...
    /**
     * Creates a new effect that will handle any failure that this effect might contain and will be recovered
     * with the effect evaluated by the specified lambda. If this effect succeeds, the new effect will also succeed
     * with the same value. If this effect fails, the specified lambda is applied to the exception to produce
     * a new effect for the new effect.
     *
    ...
    /**
     * Creates a new effect that will handle any failure that this effect might contain and will be recovered
     * with the effect evaluated by the specified lambda. The lambda computes the new effect in a thread from
     * the given executor. If this effect succeeds, the new effect will also succeed with the same value.
     * If this effect fails, the specified lambda is applied to the exception to produce a new effect for
     * the new effect.
     *
     * @param fn       the lambda to apply if this effect fails, taking the exception as input and producing a new effect.
     * @param executor the executor responsible for evaluating the lambda.
     * @return a new effect representing the original value or the result of applying the lambda in case of failure.
     */
     ...
     * new effect fails again, the new failure is ignored, and the original failure is
     * returned (this is different from {@link #recoverWith(Lambda) recoverWith} which would return the new failure).
     *
     * @param lambda the lambda to apply if this effect fails, producing a new effect.
     * @return a new effect representing either the original value or the result of applying the lambda in case of failure.
     */
    ...
    /**
     * Creates a new effect that passes the exception to the specified failConsumer in case of failure.
     * The given consumer is responsible for handling the exception and can't fail itself. If it fails,
     * the exception would be just printed out on the console or handled in another appropriate manner.
     *
    ...
    /**
     * Creates a new effect that passes the computed value to the specified successConsumer in case of success.
     * The given consumer is responsible for handling the value and can't fail itself. If it fails,
     * the exception would be just printed out on the console or handled in another appropriate manner.
     *
     ...
     * any possible failure to the specified failureConsumer. The given consumers are responsible
     * for handling the value and failure, respectively, and they can't fail themselves. If they fail,
     * the exception would be just printed out on the console or handled in another appropriate manner.
     *
     ...
     * @return a new effect representing the original value or the result of applying the consumers in case of success or failure.
     */
     ...
     * any possible failure to the specified failureConsumer. The given consumers are responsible
     * for handling the value and failure, respectively, and they can't fail themselves. If they fail,
     * the exception would be just printed out on the console.
     ...
     * compute any value within the specified time duration.
     *
     * @param time the time amount, which must be greater than 0.
     * @param unit the time unit representing the time duration.
     * @return a new effect representing the original value if computed within the timeout, or a failure with a TimeoutException.
     ...
     * this effect fails and the failure satisfies the given predicate. If a delay before the retry is imposed by the policy,
     * a thread from the fork join pool will execute the retry; otherwise (delay is zero), the same thread as the one computing
     ...
     * Creates a new effect that will retry the computation according to the specified {@link RetryPolicy policy} if
     * this effect fails and the failure satisfies the given predicate. If a delay before the retry is imposed by the policy,
     * a thread from the specified executor will execute the retry; otherwise (delay is zero), the same thread as the one computing
     * this effect will execute the retry.
     *
     * @param predicate the predicate that determines if the failure should be retried.
     * @param policy    the retry policy specifying the retry behavior.
    ...
    /**
     * Creates a new effect that will retry the computation according to the specified {@link RetryPolicy policy} if
     * this effect fails. If a delay before the retry is imposed by the policy,
     * a thread from the specified executor will execute the retry; otherwise (delay is zero), the same thread as
    ...
    /**
     * Creates a new effect that repeats the computation according to the specified {@link RetryPolicy policy} if
     * the result, when computed, satisfies the given predicate. If a delay before the retry is imposed by the policy,
     * thread from the fork join pool will execute the retry; otherwise (delay is zero), the same thread as the one computing
     * this effect will execute the retry.
     *
     ...
     * Creates a new effect that repeats the computation according to the specified {@link RetryPolicy policy} if
     * the result, when computed, satisfies the given predicate. If a delay before the retry is imposed by the policy,
     * a thread from the given executor will execute the retry; otherwise (delay is zero), the same thread as the one computing
     * this effect will execute the retry.
     *
     * @param predicate the predicate that determines if the result should be computed again.
     * @param policy    the retry policy specifying the repeat behavior.
    ...
    /**
     * Creates a copy of this effect that generates an {@link jdk.jfr.consumer.RecordedEvent} from the result of
     * the computation and sends it to the Flight Recorder system. Customization of the event can be achieved
     * using the {@link #debug(EventBuilder)} method.
     *
     ...
     * Creates a copy of this effect that generates an {@link jdk.jfr.consumer.RecordedEvent} from the result of
     * the computation and sends it to the Flight Recorder system. Customization of the event can be achieved
     * using the provided {@link EventBuilder}.
     *
