 ...
 *
 * <p>In MongoDB, a query document is used to specify which documents in a collection should be updated, and an updateCommands document defines how the updates should be performed. The `QueryUpdate` class encapsulates both the query and updateCommands documents for use in updateCommands operations, making it a convenient way to express these two key aspects of MongoDB updates.</p>
 *
 ...
 */
public record QueryUpdate(JsObj query, JsObj updateCommands) {
    ...
    public QueryUpdate {
        ...
        if(updateCommands == null) throw new IllegalArgumentException("updateCommands is null");
    }
}
