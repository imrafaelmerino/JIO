...
public interface MongoLambda<I, O> extends BiLambda<ClientSession, I, O> {
    ...
    default Lambda<I, O> standalone() {
        return i -> apply(null, i);
    }

    /**
     * Chains this MongoLambda with another, producing a new MongoLambda. The resulting MongoLambda is executed within
     * the same MongoDB client session, and the output of this operation is passed as input to the next one.
     *
     ...
     */
    default <B> MongoLambda<I, B> then(final MongoLambda<O, B> other) {
        Objects.requireNonNull(other);
        return (session, i) -> this.apply(session, i)
                                   .then(n -> other.apply(session, n));
    }

    /**
     * Chains this MongoLambda with a non-transactional Lambda, producing a new MongoLambda. The resulting MongoLambda
     * is executed within the same MongoDB client session, and the output of this operation is passed as input to the
     * Lambda.
     *
     ...
     */
    default <B> MongoLambda<I, B> then(final Lambda<O, B> other) {
        Objects.requireNonNull(other);
        return (session, i) -> this.apply(session, i)
                                   .then(other);
    }
     ...
     * @return a new MongoLambda that represents the application of the mapping function to the output of this
     * operation.
     */
    default <C> MongoLambda<I, C> map(final Function<O, C> map) {
        Objects.requireNonNull(map);
        return (session, i) -> this.apply(session, i).map(map);
    }
