...
import static jio.mongodb.MongoEvent.OP.REPLACE_ONE;
...
public final class ReplaceOne extends Op implements MongoLambda<QueryReplace, UpdateResult> {
    ...
    /**
     * Creates a `ReplaceOne` instance for performing replace one operations on a MongoDB collection with the result as
     * a JsObj.
     *
     * @param collection The supplier for the MongoDB collection.
     ...
     */
    public ReplaceOne withExecutor(final Executor executor) {
     ...
     */
    @Override
    public IO<UpdateResult> apply(final ClientSession session, final QueryReplace queryReplace) {
