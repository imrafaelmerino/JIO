...
/**
 * Represents a MongoDB find one and updateCommands operation to atomically updateCommands a single document in a collection
 * asynchronously using {@link jio.BiLambda lambdas}. This class allows you to specify a filter query criteria and an
 * updateCommands document as {@link jsonvalues.JsObj}, along with options for controlling the updateCommands behavior, such as sort
 ...
 * updateCommands options and execution behavior. You can create instances of this class with the specified collection supplier,
 * set updateCommands options, and choose an executor for asynchronous execution.
 * <p>
 ...
 * execution, and disable the recording of Java Flight Recorder (JFR) events if needed. The find one and updateCommands
 * operation requires a query filter as well as an updateCommands document to modify the matched document.
 *
...
public final class FindOneAndUpdate extends Op implements MongoLambda<QueryUpdate, JsObj> {
    ...
    /**
     * Constructs a new `FindOneAndUpdate` instance with the specified collection supplier and default updateCommands options.
     *
    ...
    /**
     * Creates a new instance of `FindOneAndUpdate` with the specified MongoDB collection supplier and default updateCommands
     * options.
     *
    ...
    /**
     * Specifies an executor to be used for running the find one and updateCommands operation asynchronously.
     *
