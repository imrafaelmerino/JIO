...
import static jio.mongodb.MongoEvent.OP.AGGREGATE;

/**
 * A class for performing aggregation operations on a MongoDB collection.
 * <p>
 * This class represents an aggregation operation, using a list of Bson pipeline stages, to be applied to a MongoDB collection.
 * The aggregation operation can be executed within a MongoDB client session if one is provided.
 * <p>
...
public final class Aggregate extends Op implements MongoLambda<List<Bson>, AggregateIterable<JsObj>> {
     ...
     */
    public Aggregate withExecutor(final Executor executor) {
     ...
     */
    @Override
    public IO<AggregateIterable<JsObj>> apply(final ClientSession session, final List<Bson> stages) {
        ...
        Supplier<AggregateIterable<JsObj>> supplier =
                eventWrapper(() -> {
                                 ...
                                 return session == null ?
                                         ...
                                         collection.aggregate(session, stages);
                             },
                             AGGREGATE
                            );
