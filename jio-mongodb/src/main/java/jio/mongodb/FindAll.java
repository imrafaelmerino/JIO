...
/**
 * Represents a MongoDB find operation to retrieve all documents in a collection using
...
public final class FindAll extends Find implements MongoLambda<FindBuilder, FindIterable<JsObj>> {
     ...
     */
    public FindAll withExecutor(final Executor executor) {
     ...
     */
    @Override
    public IO<FindIterable<JsObj>> apply(final ClientSession session, final FindBuilder findBuilder) {
        return query(session, findBuilder);
    }
}
